...
local function canUseSpell(spellVocations, playerVocation, spellId)
  -- Verificação por AllowedSpellsByClass (id+classe)
  if spellId and AllowedSpellsByClass and AllowedSpellsByClass[spellId] then
    if AllowedSpellsByClass[spellId][playerVocation] then
      return true
    else
      return false
    end
...
function updateSpelllist()
    ...
    if not localPlayer then
      print('DEBUG: localPlayer is nil!')
      return
    end
    print('DEBUG: Player vocation id = ' .. tostring(localPlayer:getVocation()))
  for spellName, info in pairs(SpellInfo[SpelllistProfile]) do
    print('DEBUG: Spell candidate: name=' .. tostring(spellName) .. ' id=' .. tostring(info.id))
    local tmpLabel = spellList:getChildById(spellName)
    local localPlayer = g_game.getLocalPlayer()

    local vocationId = localPlayer:getVocation()
    local allowed = false
    if AllowedSpellsByClass and AllowedSpellsByClass[info.id] then
      allowed = AllowedSpellsByClass[info.id][vocationId] or false
    end
    if not _G._debugSpellShownCount then _G._debugSpellShownCount = 0 end
    if allowed then
      tmpLabel:setVisible(true)
      _G._debugSpellShownCount = _G._debugSpellShownCount + 1
    else
      tmpLabel:setVisible(false)
    end
  end
  if _G._debugSpellShownCount then
    print("DEBUG: Total spells shown = " .. tostring(_G._debugSpellShownCount))
    _G._debugSpellShownCount = 0
  end
end
